...
pub fn render(voxels: impl IntoIterator<Item = Voxel>) -> Vec<Triangle<XY>> {
    ...
    for vox in voxels {
        match faces.entry(project_ij(vox.0)) {
            ...
            Entry::Occupied(mut o) => {
                if vox.nearness() > o.get().nearness() {
                    o.insert(vox);
                }
            }
        }
    ...
    voxels.sort_unstable_by_key(|v| Reverse(v.nearness()));
    ...
    for vox in voxels {
        for triangle in vox.triangles(&spatial_ix) {
...
impl Voxel {
    fn triangles(&self, voxels: &HashSet<P3>) -> [Triangle<P3>; 6] {
        let (x, y, z) = self.0;
